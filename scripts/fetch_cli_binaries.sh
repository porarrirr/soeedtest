...
extract_speedtest "$OOKLA_CLI_AARCH64_TGZ_URL" "$ARM64_DIR" "$ARM64_BUNDLE_DIR"

if [[ -n "${OOKLA_CLI_ARMHF_TGZ_URL:-}" ]]; then
  mkdir -p "$ARMV7_DIR"
  mkdir -p "$ARMV7_BUNDLE_DIR"
  extract_speedtest "$OOKLA_CLI_ARMHF_TGZ_URL" "$ARMV7_DIR" "$ARMV7_BUNDLE_DIR"
fi

if [[ -n "${OOKLA_CLI_X86_64_TGZ_URL:-}" ]]; then
  mkdir -p "$X86_64_DIR"
  mkdir -p "$X86_64_BUNDLE_DIR"
  extract_speedtest "$OOKLA_CLI_X86_64_TGZ_URL" "$X86_64_DIR" "$X86_64_BUNDLE_DIR"
fi

if [[ -n "${OOKLA_CLI_X86_TGZ_URL:-}" ]]; then
  mkdir -p "$X86_DIR"
  mkdir -p "$X86_BUNDLE_DIR"
  extract_speedtest "$OOKLA_CLI_X86_TGZ_URL" "$X86_DIR" "$X86_BUNDLE_DIR"
fi
